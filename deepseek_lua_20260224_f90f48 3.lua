...
-- ==================== NEW: Hook the suggestion system ====================
task.spawn(function()
    task.wait(2) -- Wait for modules to load
    pcall(function()
        -- Try to hook the suggestion system
        local Success, Result = pcall(function()
            -- Look for the suggestion function in BackpackApp
            if BackpackApp and BackpackApp.suggest_items then
                local originalSuggest = BackpackApp.suggest_items
                BackpackApp.suggest_items = function(self, ...)
                    -- When suggest is clicked, just open the normal pick_item
                    if mockState.active and mockState.trade then
            ...
            -- Also try to hook any remote events related to suggestions
            local SuggestionRemote = ReplicatedStorage:FindFirstChild('Remotes') 
                and ReplicatedStorage.Remotes:FindFirstChild('TradeAPI') 
                and ReplicatedStorage.Remotes.TradeAPI:FindFirstChild('SuggestItem')
            
            if SuggestionRemote then
                local oldFire = SuggestionRemote.FireServer
                SuggestionRemote.FireServer = function(self, ...)
                    if mockState.active and mockState.trade then
                        -- Instead of firing server, just open pick_item
                        BackpackApp:pick_item({
                            keep_cached_scroll_positions_on_open = true,
                            allow_callback = function() return true end
                        })
                        return
                ...
                print("✅ Hooked SuggestItem remote")
            end
            
            -- Also hook the chat suggestion button if it exists
            local ChatService = Services.Chat
            if ChatService then
                local oldChat = ChatService.Chat
                -- This is a generic attempt - might need adjustment based on actual structure
                print("✅ Chat service found")
            end
        end)
        
        if Success then
            print("✅ Successfully hooked suggestion system")
        else
            print("⚠️ Could not hook suggestion system - Suggest button may not work")
        end
    end)
end)
