...
local function sendTradeChatMessage(message)
    ...
    if TradeApp and TradeApp._render_message_in_trade_chat then
        -- FIXED: Send only the message without partner name prefix
        TradeApp:_render_message_in_trade_chat(nil, message, true)
        return true
    end
    return false
end
...
-- ==================== MODIFIED: _add_item_to_my_offer to handle suggestions ====================
TradeApp._add_item_to_my_offer = function(self, ...)
    print("📦 _add_item_to_my_offer called with args:", ...)
    if mockState.active and mockState.trade then
        if mockState.isAddingItem then return end
        mockState.isAddingItem = true
        
        -- Check if called with arguments (might be from suggestion)
        local args = {...}
        local pickedItem = nil
        
        -- If we received an item directly, use it
        if args and args[1] and type(args[1]) == "table" and args[1].unique then
            pickedItem = args[1]
            print("✅ Received item directly from suggestion")
        else
            -- Otherwise open backpack picker
            print("📂 Opening backpack picker")
            pcall(function()
        ...
        if pickedItem then
            print("✅ Item selected:", pickedItem.unique)
            ...
            if not alreadyInTrade then
                ...
                -- Show success hint
                if HintApp then
                    HintApp:hint({ text = 'Item added to trade!', length = 2, overridable = true })
                end
            else
        ...
        print("⚠️ Not in mock trade, calling original")
        return mockState.originalFunctions._add_item_to_my_offer(self, ...)
    end
end

-- Keep the rest of the hook functions
...
-- ==================== IMPROVED: Hook the suggestion system ====================
task.spawn(function()
    task.wait(2) -- Wait for modules to load
    pcall(function()
        -- Method 1: Hook the actual suggest function in BackpackApp
        if BackpackApp then
            -- Try to find the suggest method
            if BackpackApp.suggest_items then
                local originalSuggest = BackpackApp.suggest_items
                BackpackApp.suggest_items = function(self, ...)
                    print("📝 Suggest button clicked - opening backpack")
                    if mockState.active and mockState.trade then
                        -- Instead of suggesting, just open the normal pick_item
                        local success, item = pcall(function()
                            return self:pick_item({
                                keep_cached_scroll_positions_on_open = true,
                                allow_callback = function() return true end,
                                title = "Select an item to suggest"
                            })
                        end)
                        if success and item then
                            -- If an item was picked, add it to the trade
                            if TradeApp and TradeApp._add_item_to_my_offer then
                                TradeApp:_add_item_to_my_offer(item)
                            end
                        end
                        return
                    end
                    return originalSuggest and originalSuggest(self, ...)
                end
                print("✅ Hooked BackpackApp.suggest_items")
            end
            
            -- Also hook the pick_item method to ensure it works
            if BackpackApp.pick_item then
                local originalPickItem = BackpackApp.pick_item
                BackpackApp.pick_item = function(self, options)
                    if mockState.active and mockState.trade then
                        -- Ensure options are set correctly for suggestion context
                        options = options or {}
                        options.keep_cached_scroll_positions_on_open = true
                        options.allow_callback = function() return true end
                        return originalPickItem(self, options)
                    end
                    return originalPickItem(self, options)
                end
                print("✅ Enhanced BackpackApp.pick_item")
            end
        end
        
        -- Method 2: Hook any remote events related to suggestions
        local function findAndHookRemotes()
            -- Look in various places for suggestion-related remotes
            local searchPaths = {
            ...
            for _, path in ipairs(searchPaths) do
                if path then
                    -- Look for TradeAPI suggestions
                    local tradeAPI = path:FindFirstChild('TradeAPI')
                    if tradeAPI then
                        local suggestRemote = tradeAPI:FindFirstChild('SuggestItem') or 
                                              tradeAPI:FindFirstChild('SuggestItems') or
                                              tradeAPI:FindFirstChild('MakeSuggestion')
                        if suggestRemote then
                            local oldFire = suggestRemote.FireServer
                            suggestRemote.FireServer = function(self, ...)
                                print("📡 Suggestion remote fired - intercepting")
                                if mockState.active and mockState.trade then
                                    -- Instead of firing server, open backpack
                                    if BackpackApp and BackpackApp.pick_item then
                                        local item = BackpackApp:pick_item({
                                            keep_cached_scroll_positions_on_open = true,
                                            allow_callback = function() return true end
                                        })
                                        if item and TradeApp and TradeApp._add_item_to_my_offer then
                                            TradeApp:_add_item_to_my_offer(item)
                                        end
                                    end
                                    return
                            ...
                            print("✅ Hooked suggestion remote:", suggestRemote.Name)
                        end
                    end
                    
                    -- Look for general suggestion remotes
                    local suggestRemote = path:FindFirstChild('SuggestItem') or 
                                          path:FindFirstChild('MakeSuggestion')
                    if suggestRemote and not suggestRemote.Parent.Name == "TradeAPI" then
                        local oldFire = suggestRemote.FireServer
                        suggestRemote.FireServer = function(self, ...)
                            if mockState.active and mockState.trade then
                                if BackpackApp and BackpackApp.pick_item then
                        ...
                        print("✅ Hooked general suggestion remote:", suggestRemote.Name)
                    end
                end
            end
        end
        ...
        -- Method 3: Hook the UI button itself if possible
        task.spawn(function()
            task.wait(3)
            pcall(function()
                -- Try to find the suggest button in the chat UI
                local playerGui = Players.LocalPlayer:WaitForChild('PlayerGui')
                local chatFrame = playerGui:FindFirstChild('Chat') or 
                                  playerGui:FindFirstChild('ChatApp') or
                                  (playerGui:FindFirstChild('TradeApp') and playerGui.TradeApp:FindFirstChild('ChatFrame', true))
                
                if chatFrame then
                    -- Look for buttons with "suggest" in their text or name
                    local function scanForSuggestButton(obj)
                        if obj:IsA('TextButton') and (obj.Text:lower():find('suggest') or obj.Name:lower():find('suggest')) then
                            print("🔍 Found suggest button:", obj.Name)
                            -- Clone and override if needed, but careful with this
                        end
                        for _, child in ipairs(obj:GetChildren()) do
                            scanForSuggestButton(child)
                        end
                    end
                    scanForSuggestButton(chatFrame)
                end
            end)
        end)
        
        print("✅ Suggestion system hooks installed - Suggest button should now open backpack")
    end)
end)
